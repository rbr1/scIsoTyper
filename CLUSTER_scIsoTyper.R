...
if(a==1){
  ...
  batch = "PDAC150Ka"
  out_dir = "/well/immune-rep/shared/10X_GENOMICS/PDAK150K_WORKING_DATA/VDJ_CHECK/"
}
...
Move_files_to_working_directory<-function(TCR.location, BCR.location, sample_id, out_dir){
  for(c in BCRs){
    if(is.na(BCR.location[c])==F){
      sample = sample_id[c]
      fasta_file = gsub("_annotations.csv",".fasta",BCR.location[c] )
      csv_file = BCR.location[c]
      command = concat(c("cp ", csv_file, " ", out_dir, "filtered_contig_annotations_BCR_", sample,".csv"))
      system(command)
      command = concat(c("cp ", fasta_file, " ", out_dir, "filtered_contig_BCR_", sample,".fasta"))
      system(command)
  ...
  for(c in TCRs){
    if(is.na(TCR.location[c])==F){
      ...
      fasta_file = gsub("_annotations.csv",".fasta",TCR.location[c] )
      csv_file = TCR.location[c]
      command = concat(c("cp ", csv_file, " ", out_dir, "filtered_contig_annotations_TCR_", sample,".csv"))
      system(command)
      command = concat(c("cp ", fasta_file, " ", out_dir, "filtered_contig_TCR_", sample,".fasta"))
      system(command)
      print(concat(c(c, " ", sample)))}}
}

Move_files_to_working_directory(TCR.location, BCR.location, sample_id, out_dir) ### this will print out the sample IDs that it has moved

################# batch for IMGT

Group_together_fasta_files_for_IMGT<-function(TCR.location, BCR.location, sample_id, out_dir,batch){
  ...
  outfile = concat(c(out_dir, "IMGT_sample_ID_mapping.txt"))
  ...
  library(ape)
  outfile = concat(c(out_dir, "All_filtered_contig_BCR_", batch,"_"))
  index = 1
  total_seqs = NULL
  ...
  for(c in BCRs){
    if(is.na(BCR.location[c])==F){
      ...
      fasta_file = concat(c(out_dir, "filtered_contig_BCR_", sample,".fasta"))
  ...
  outfile = concat(c(out_dir, "All_filtered_contig_TCR_", batch,"_"))
  index = 1
  total_seqs = NULL
  max_n_sequences_per_file = 1000000-10
  for(c in TCRs){
    if(is.na(TCR.location[c])==F){
      sample = sample_id[c]
      unique_ID = out[match(sample,out[,1]),"unique_short_ID"]
      fasta_file = concat(c(out_dir, "filtered_contig_TCR_", sample,".fasta"))
      seqs <- read.dna(fasta_file, format = "fasta", as.character = T)
      names(seqs) = paste0(names(seqs),"||", unique_ID, sep = "")
      if( length(seqs) + length(total_seqs)> max_n_sequences_per_file){
        write.dna(total_seqs, concat(c(outfile,index,".fasta")), format = "fasta", append = FALSE)
...
Group_together_fasta_files_for_IMGT(TCR.location, BCR.location, sample_id, out_dir,batch) ## this will print out the cumulative number of sequences per sample and the location of the output file
...
Map_annotations_by_sample<-function(dir_IMGT_TCR, BCRs, TCRs,dir_IMGT_BCR, sample_id){
  file = concat(c(out_dir, "IMGT_sample_ID_mapping.txt"))
  ...
  long_ID1 = long_ID
  names(long_ID1) = long_ID
  length_names = 21
  library(stringr)
  names(long_ID1) = str_trunc(names(long_ID1), 21, "right", ellipsis = "")
  
  ###################################################
  types = c("BCR","TCR")
  files = c(concat(c(dir_IMGT_BCR,"2_IMGT-gapped-nt-sequences.txt")),concat(c(dir_IMGT_TCR,"2_IMGT-gapped-nt-sequences.txt")))
  chain_name1 = c("IGH","TRA")
  chain_name2 = c("IGL","TRB")
  for(f in c(1:length(files))){
    ...
    seq1 = gsub(".","",seq1,fixed = T)
    len1= str_length(seq1)
    #w=which(len==0)
    #seq[w] = seq1[w]
    #seq = gsub(".","",seq,fixed = T)
    ...
    which(is.na(orig_sample))
    
    orig_samples = sort(unique(orig_sample))
    orig_samples[which(orig_samples %in% names(long_ID)==F)]
    sample_long_ID = long_ID[orig_sample]
    orig_sample[which(is.na(sample_long_ID))]
    ...
    for(c in BCRs){
      if(is.na(BCR.location[c])==F){
        sample = sample_id[c]
        fasta_file = concat(c(out_dir, "/filtered_contig_", type,"_", sample,".fasta"))
        seqs <- read.dna(fasta_file, format = "fasta", as.character = T)
        ...
        if(length(seq_inds)>0){
          ...
          outfile = concat(c(out_dir, "/IMGT_filtered_contig_", type,"_", sample,".fasta"))
  ...
  for(f in c(1:length(files))){
    ...
    for(c in BCRs){
      if(is.na(BCR.location[c])==F){
        ...
        found = c(found, length(seq_inds))
        outfile = concat(c(out_dir, "IMGT_filtered_annotation_", type,"_", sample,".txt"))
  ...
  for(f in c(1:length(files))){
    ...
    for(c in BCRs){
      if(is.na(BCR.location[c])==F){
        ...
        found = c(found, length(seq_inds))
        if(length(seq_inds)>0){
          outfile = concat(c(out_dir, "IMGT_filtered_amino_acids_", type,"_", sample,".fasta"))
          write.dna(total_seqs, outfile, format = "fasta", append = FALSE, nbcol = -1,colw = 10000000)
          used = c(used,seq_inds)
          print (c(sample, length(w1)))
        }}}
...
## for others using this on their own datasets, you may need to check that the sample ID mapping works for you. Some of this is dataset-specific. 
...
for(c in BCRs){
  if(is.na(BCR.location[c])==F){
    sample = sample_id[c]
    fasta_file = concat(c(out_dir, "/IMGT_filtered_contig_", type,"_", sample,".fasta"))
    csv_file = BCR.location[c]
    gene = "IG"
    filtered_fasta_contig_file = concat(c(out_dir, "Sequences_",sample,"_",gene))
    immune_cell_annotation_file = concat(c(out_dir, "Cell_annotation_",sample,"_",gene,".txt"))
    trimmed_sequences_VDJ = concat(c(out_dir, "IMGT_filtered_contig_",sample,"_",gene))
    IMGT_filtered_annotation = concat(c(out_dir,"IMGT_filtered_annotation_",type,"_",sample,".txt"))
...
for(o in c(1:length(Overall_sample_groups))){
  ...
  p=which(match(samples_group, sample_id) %in% BCRs)
  samples_group = samples_group[p]
  print(o)
  print(Overall_sample_groups[o])
  print(cbind(samples_group))
  ...
  if(length(samples_group)>0){
    for(ch in c(1:length(chains))){
      print(ch)
      chain = chains[ch]
      samples_ids = paste0(samples_group, collapse = ",")
      sample_fastas = paste0(concat(c(out_dir,"IMGT_filtered_contig_BCR_")), samples_group,".fasta" )
      amino_acid_fastas = paste0(concat(c(out_dir, "IMGT_filtered_amino_acids_BCR_")), samples_group,".fasta" )
      cell_info_files = paste0(concat(c(out_dir,"Cell_annotation_")), samples_group,"_IG.txt" )
      ## check (w1, w2, w3 will be empty if all the files exist)
      w1 = which(file.info(sample_fastas)$size == 0)
      w2 = which(file.info(amino_acid_fastas)$size == 0)
      w3 = which(file.info(cell_info_files)$size == 0)
      
      id = concat(c(grouped_sample_id ,"_",chain))
      
      combined_sequence_file = concat(c( out_dir,"Sequences_combined_",Overall_sample_groups[o],"_",chain,".fasta"))
      tmp_file = concat(c( out_dir,"Tmp_",Overall_sample_groups[o],"_",chain))
      output_file_prefix = concat(c( out_dir,"Edges_",Overall_sample_groups[o],"_",chain))
      cluster_file = concat(c( out_dir,"Cluster_identities_",Overall_sample_groups[o],"_",chain,".txt"))
...
for(o in c(1:length(Overall_sample_groups))){
  ...
  #check which have BCRs/TCRs
  p=which(match(samples_group, sample_id) %in% TCRs)
  samples_group = samples_group[p]
  print(o)
  print(Overall_sample_groups[o])
  print(cbind(samples_group))
  ...
  if(length(samples_group)>0){
    for(ch in c(1:length(chains))){
      ...
      chain = chains[ch]
      samples_ids = paste0(samples_group, collapse = ",")
      sample_fastas = paste0(concat(c(out_dir,"IMGT_filtered_contig_TCR_")), samples_group,".fasta" )
      amino_acid_fastas = paste0(concat(c(out_dir, "IMGT_filtered_amino_acids_TCR_")), samples_group,".fasta" )
      cell_info_files = paste0(concat(c(out_dir,"Cell_annotation_")), samples_group,"_TCR.txt" )
      ## check (w1, w2, w3 will be empty if all the files exist)
      w1 = which(file.info(sample_fastas)$size == 0)
      w2 = which(file.info(amino_acid_fastas)$size == 0)
      w3 = which(file.info(cell_info_files)$size == 0)
...
for(c in c(1:length(sample_id))){
  if(c %in% BCRs){
    sample = sample_id[c]
    cell_info_BCR = paste0(concat(c(out_dir,"Cell_annotation_")), sample,"_IG.txt" )
  ...
  if(c %in% TCRs){
    sample = sample_id[c]
    cell_info_TCR = paste0(concat(c(out_dir,"Cell_annotation_")), sample,"_TCR.txt" )
...
for(o in c(1:length(Overall_sample_groups))){
  cluster_file1 = paste0(concat(c(out_dir,"Cluster_identities_")), Overall_sample_groups[o],"_TRA.txt" )
  cluster_file2 = paste0(concat(c(out_dir,"Cluster_identities_")), Overall_sample_groups[o],"_TRB.txt" )
...
for(o in c(1:length(Overall_sample_groups))){
  cluster_file3 = paste0(concat(c(out_dir,"Cluster_identities_")), Overall_sample_groups[o],"_IGH.txt" )
  cluster_file4 = paste0(concat(c(out_dir,"Cluster_identities_")), Overall_sample_groups[o],"_IGL.txt" )
...
saveRDS(file=concat(c(out_dir,"/VDJ_information_", batch,".VDJ")), VDJ_object)

## output as matrix file
out_file_table = concat(c(out_dir,"/VDJ_information_BCR_", batch,".txt"))
write.table(m_VDJ_BCR, file = out_file_table, append = FALSE, quote = FALSE, sep = "\t",eol = "\n", na = "NA", dec = ".", row.names = T,col.names = T, qmethod = c("escape", "double"),fileEncoding = "")
out_file_table = concat(c(out_dir,"/VDJ_information_TCR_", batch,".txt"))
...
VDJ_object = readRDS(file=concat(c(out_dir,"/VDJ_information_", batch,".VDJ")))
...
out_file_table = concat(c(out_dir,"/VDJ_information_n_droplets_", batch,".txt"))
